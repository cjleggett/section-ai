...
def search(source, dest):
    ...
    # Create node from source value, create frontier, add Node
    current = Node(source, 0, None)
    queue = PrioQueue()
    queue.add_node(current)

    # Repeat indefinitely
    while True:

        # If we've reached the destination, print Yay!
        if current.value == dest:
            print("Yay")
            return

        # Look at all neighbors of the node, and add them to queue
        for (value, weight) in graph[current.value]:
            new_node = Node(value, weight + current.priority, current)
            queue.add_node(new_node)

        # Choose a new node as the current one
        current = queue.pop_node()

        # If there are no nodes in the frontier, there is no path
        if not current:
            print("Awwwww")
            return 
